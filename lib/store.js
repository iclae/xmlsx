...
function style(s) {
  ...
  this.bound(_getEdgeByCell(cell, false))
}
...
function parse() {
  ...
  self.edge = _getEdgeByData(newData)
...
function _getEdgeByData(entries) {
  return {
...
function _getEdgeByCell(cell, fromZero) {
  return {
...
function _getCell(x, y) {
  var A = 65
  return String.fromCharCode(A + x) + String(y + 1)
}
